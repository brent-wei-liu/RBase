...
int main(int argc, char** argv){
	...
	bool runBBS = false;
	bool runBBSdyn = false;
	bool runBBSorth = false;
...
//		cerr << "Outputing statistics..." << endl;
	...
	// BBSdyn
	if (runBBSdyn){
		BBSdyn *bbs = new BBSdyn(dataset_file_name, query_file_name, numQueries, cacheSize, dimension, bufferSize);
		cerr << "Executing BBSdyn..." << endl;
		bbs->execute(BBSDYN, withCache);
//		cerr << "Outputing statistics..." << endl;
		cerr << bbs->getStatistics(BBSDYN);
		timeOut << bbs->query_time << "\t";
		totimeOut << bbs->total_time << "\t";
		iosOut << bbs->accesses << "\t";
		sizeOut << bbs->numResults << "\t";
	...
	if (runBBSorth){
		BBSdyn *bbs = new BBSdyn(dataset_file_name, query_file_name, numQueries, cacheSize, dimension, bufferSize);
		cerr << "Executing BBSorth..." << endl;
		bbs->execute(BBSORTH, withCache);
//		cerr << "Outputing statistics..." << endl;
		cerr << bbs->getStatistics(BBSORTH);
		timeOut << bbs->query_time << "\t";
		totimeOut << bbs->total_time << "\t";
		iosOut << bbs->accesses << "\t";
		sizeOut << bbs->numResults << "\t";
